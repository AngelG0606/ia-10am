...
class Juego:
    def __init__(self) -> None:
        ...
        self.crouch_timer = 0
        self.crouch_duration = 20  # ~0.18 s a 45 fps — corto para permitir múltiples agachadas
    ...
    def exportar_datos_csv(self) -> str:
        ...
        try:
            with open(ruta, "w", newline="", encoding="utf-8") as f:
                writer = csv.writer(f)
                writer.writerow(["velocidad_bala", "distancia", "altura_bala", "estado_anterior", "frames_estado", "accion"])
                for s in self.datos_modelo:
                    writer.writerow([s.velocidad_bala, s.distancia, s.altura_bala, s.estado_anterior, s.frames_estado, s.accion])
    ...
    def disparar_bala(self) -> None:
        if not self.bala_disparada:
            ...
            # Reset temporal state para el modelo con cada nueva bala
            self.estado_modelo = 0
            self.frames_estado_modelo = 0
    ...
    def registrar_decision_manual(self) -> None:
        ...
        estado_ant = getattr(self, "estado_modelo", 0)
        ...
        if accion_real == estado_ant:
            self.frames_estado_modelo = frames_ant + 1
        else:
            self.estado_modelo = accion_real
    ...
    def entrenar_modelo(self) -> Tuple[bool, str]:
        ...
        # Features: velocidad_bala, distancia, altura_bala, estado_anterior, frames_estado
        X = [[s.velocidad_bala, s.distancia, s.altura_bala, s.estado_anterior, s.frames_estado] for s in samples]
    ...
    def decision_auto(self) -> int:
        ...
        X = [[float(self.velocidad_bala), float(distancia), float(self.altura_bala_relativa), float(estado_ant), float(frames_ant)]]
        ...
        if accion == estado_ant:
            self.frames_estado_modelo = frames_ant + 1
        else:
            self.estado_modelo = accion
            self.frames_estado_modelo = 1
